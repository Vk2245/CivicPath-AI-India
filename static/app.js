/**
 * app.js — CivicPath Frontend Application
...
// ═══ ONBOARDING QUIZ ═══
const quizQuestions = [
  { id: 'state', question: 'What state do you live in?', type: 'select',
    options: ['Bihar','Uttar Pradesh','Maharashtra','West Bengal','Tamil Nadu','Karnataka','Gujarat','Delhi','Kerala','Other'] },
  { id: 'registered', question: 'Are you registered to vote?', type: 'choice',
  ...
  { id: 'election_type', question: 'What election are you preparing for?', type: 'choice',
    options: [{ label: '🗳️ Lok Sabha (General)', value: 'general' }, { label: '🔵 Vidhan Sabha (State Assembly)', value: 'assembly' },
              { label: '🏛️ Municipal/Panchayat (Local)', value: 'local' }, { label: '📋 By-Election', value: 'bypoll' }] },
  ...
  container.innerHTML = journeySteps.map((step, i) => {
    ...
    return '<div class="timeline-step" role="listitem" aria-label="Step ' + step.step_number + ': ' + step.title + '">' +
      ...
      (step.status !== 'completed' ?
        '<button class="btn btn-primary btn-sm" onclick="completeStep(' + i + ')" aria-label="Mark step ' + step.step_number + ' complete">✓ Complete</button>' : '<span class="badge" style="color:var(--green)">✓ Done</span>') +
...
async function completeStep(index) {
  ...
  renderTimeline();
  showToast('Step ' + journeySteps[index].step_number + ' completed! 🎉');
...
async function readStepAloud(index) {
  ...
  if ('speechSynthesis' in window) {
    ...
    u.lang = currentLanguage === 'en' ? 'en-US' : currentLanguage;
...
function toggleVoice() {
  ...
  navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
    ...
    mediaRecorder.onstop = async () => {
      ...
      showToast('Processing voice...', 'success');
      try {
...
async function checkMyth() {
  ...
  if (!text || text.length < 5) { showToast('Enter a claim to fact-check', 'error'); return; }
  const resultDiv = document.getElementById('myth-result');
  resultDiv.innerHTML = '<div style="text-align:center;padding:20px;"><div class="spinner" style="margin:0 auto;"></div><p style="margin-top:12px;">Analyzing with Google Gemini AI...</p></div>';
  ...
  try {
    ...
    const icons = { myth: '❌', fact: '✅', misleading: '⚠️', unverifiable: '❓' };
    ...
    resultDiv.innerHTML = '<div class="myth-result ' + (colors[verdict] || '') + '">' +
      '<div class="myth-verdict">' + (icons[verdict] || '❓') + ' ' + verdict.toUpperCase() + '</div>' +
    ...
    resultDiv.innerHTML = '<div class="myth-result misleading"><div class="myth-verdict">⚠️ DEMO MODE</div><p>Myth detection requires Google Gemini API. In production, this would analyze your claim against official election sources.</p></div>';
  }
...
async function findPollingPlaces() {
  ...
  try {
    ...
    resultsDiv.innerHTML = places.map(p =>
      ...
      '<p style="font-size:0.875rem;">' + p.address + '</p>' +
      (p.hours ? '<p style="font-size:0.8125rem;color:var(--amber);">🕐 ' + p.hours + '</p>' : '') +
      (p.distance_miles ? '<p style="font-size:0.8125rem;color:var(--text-muted);">📏 ' + p.distance_miles + ' miles</p>' : '') +
      '</div>'
    ).join('') || '<p>No polling places found. Try a different address.</p>';
  } catch (e) {
    resultsDiv.innerHTML = '<p style="color:var(--text-secondary);">Polling place search requires Google Maps API in production.</p>';
  }
}
...
function useMyLocation() {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    try {
      ...
      const places = data.places || [];
      resultsDiv.innerHTML = places.map(p =>
        '<div class="card" style="margin-bottom:12px;"><h3 style="font-size:1rem;">' + p.name + '</h3><p style="font-size:0.875rem;">' + p.address + '</p></div>'
      ).join('') || '<p>No polling places found nearby.</p>';
