...
class TestFullUserJourney:
    ...
    def test_complete_user_flow(self, client):
        ...
        step_res = client.put(
            f"/journey/{journey_id}/step", json={"step_number": 1, "status": "completed"}
        )
        ...
        chat_res = client.post(
            "/chat",
            json={"message": "What ID do I need to vote in Bihar?", "journey_id": journey_id},
        )
        assert chat_res.status_code == 200
        assert "response" in chat_res.json()

        # Step 4: Myth check
        myth_res = client.post(
            "/chat/myth-check", json={"text": "EVMs can be hacked via bluetooth"}
        )
