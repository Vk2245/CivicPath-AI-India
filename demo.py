...
async def run_demo():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            print(
                f"❌ Server not running. Please start it with: uvicorn main:app --port 8000\nError: {e}\n"
            )
            return
        ...
        data = res.json()
        print(
            f"Status {res.status_code}: Total Services Integrated -> {data.get('total_google_services')}"
        )
        ...
        journey = res.json()
        print(
            f"Status {res.status_code}: Journey created with ID '{journey.get('journey_id', 'demo-id')}'"
        )
        ...
        chat_reply = res.json()
        print(
            f"Status {res.status_code}: AI Assistant Reply:\n> {chat_reply.get('response', 'No response')}\n"
        )

    print(
        "🎉 Demo complete! The fallback demo data works perfectly for testing without real API keys."
    )
